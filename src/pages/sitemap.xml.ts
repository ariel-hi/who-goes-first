...
export function GET() {
  ...
  const routes = ['/', '/about/', '/fairness/', '/privacy/', '/choose-who-goes-first/', '/board-games/', '/methods/spinner/', '/methods/cards/', '/methods/towers/', '/methods/straws/', '/methods/dice/', '/methods/coin/', '/methods/shells/', ...(process.env.DISABLE_BALLOON !== 'true' ? ['/methods/balloon/'] : []), ...(games.length ? ['/games/'] : []), ...(getPrompts().length ? ['/house-rules/'] : []), ...hubs];
