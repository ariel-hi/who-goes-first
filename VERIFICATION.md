...
The home random-rule action preserves its enabled native button through loading with a busy guard and ARIA state, retaining keyboard focus, retry and the user's actual native destination after tabbing away. Secure selection and shared lazy-index behavior remain intact. Root rehashed the preceding frozen defect evidence and isolated repair source/build/runtime evidence before applying exactly its three owned files.

**Required gates passed:** `npm run verify` (159 checked files, zero diagnostics, lint, **153 unit tests /16 files**, content validation, **1,122 pages /30,396 internal links**, SEO/static/private-content/security/indexing/cache audits); release matrix including configured revenue fixtures; final typecheck/lint; and **48/48 selected browser cases /77.274 seconds** across Chromium, Firefox and WebKit, zero retries/skips/flaky/unexpected results. The36 manual/identity cases cover320/1280 static no-JS articles/shelves, exact reviewed/pending links and native French lookup/Back on all three search surfaces. The12 keyboard cases cover cold/repeated/warm activation in actual light/dark themes, native away destination and failed-fetch retry. Root visually inspected six full articles and three busy/result captures across widths/engines/themes. Gzip initial JS115,696B, Balloon1,182B, basic home134,715B. No new full714-case, screen-reader, physical-device, mobile-hidden-panel or BFCache claim.

The OS-port frozen-site runner closed its owned server/child and rehashed2,123 compiled members unchanged. Final native input restores original CRLF formatting only; all parsed objects are identical to the passing compiled runtime's intermediate LF binding. Dated source seals remain intact with a separate formatting addendum and final actual getters. Preservation verifies1,914 preceding catalog/draft byte sequences,285 other native decisions,71 old sources,48 old overrides,370 old portable fingerprints and all three full previous holds unchanged. Original draft-status assertion failure remains preserved; its correction changed the assertion only. Logs `artifacts/sep26-amigo-combined-{verify,release,browser,final-check,final-lint}.log`; exact source/application/frozen-browser bindings and limits are in the complete checkpoint. Local checkpoint only; separately advancing main has different counts. Perpetual catalog/UX/SEO/economic improvement goal remains active.

## Growth integration and main promotion — 2026-09-26 UTC

Integrated growth automation `82f5c2a`, local preview checkpoints through `72b1fd0`, and current main `c83c685`. Final inventory: **5,006 identities /957 approved editions /947 sourced /4,059 pending /370 portable criteria /60 house prompts**. Exact existing source approvals are preserved; the merge introduces no new editorial approvals. See [the integration checkpoint](research/coverage/growth-merge-sep26.md).

**Required local gates passed:** `npm ci`; `npm run verify` (zero Astro diagnostics, lint, **152 unit tests /16 files**, content validation, **1,118 pages /30,311 internal links**, SEO/security/cache/redirect audits); and `npm run test:release` (empty preview/production, content fixtures, configured revenue fixtures, disabled Balloon, and invalid configuration rejection). The revenue fixture verifies ad-free picker/method/404 pages, per-rule share images, publisher links, sponsored Amazon boxes outside rule text, funding/privacy links, ads.txt and the conditional CSP/referrer policy. Initial JavaScript gzip115,673B, Balloon1,182B, basic home134,678B. Final type check and lint also passed after the assertion corrections.

**Browser coverage completed across two runs:** the full serial Chromium/Firefox/WebKit run executed714 cases:692 passed and22 failed in38.1minutes. Twenty-one failures were stale assertions for retained “Pick again” results, desktop inline random rules, answer snippets and the expanded catalog. Those assertions were corrected without removing behavior checks. One WebKit desktop case timed out waiting for radio-control stability; its source and timeout were unchanged. `npm.cmd run test:browser -- --last-failed --reporter=line,json` then passed **22/22 in3.0minutes**, with no retries or skips. Thus every one of714 cases has a passing result across the full run and focused rerun; this is not an uninterrupted714-pass claim. All four WebKit large-group cases, including desktop reduced motion, passed in the full run. Both runs required free4321/4322 ports and fresh servers with reuse disabled.

Logs: `artifacts/growth-merge-{npm-ci,verify,release,browser,browser-rerun,final-check,final-lint}.log`. Original failures and traces are preserved in `artifacts/growth-merge-initial-browser-results/`; the rerun JSON is `artifacts/growth-merge-browser-rerun.json`. Desktop game/publisher and mobile theme screenshots plus a generated share image were visually inspected; the three page layouts had no horizontal overflow. CI's job allowance is60minutes for the expanded serial suite; individual test timeouts are unchanged.

Production revenue variables were not changed. Demand priority remains missing rules in order, verified unmatched identities, then low-click titles/descriptions; primary-source review remains mandatory. The sections below retain historical checkpoints and their original limitations.

## Amigo identities and visible Quick results — 2026-09-26 UTC

Current local scope: **4,999 identities /893 approved editions /887 sourced /4,112 pending /361 portable criteria /60 house prompts**. Lama Dice, Cabanga! and Meister Makatsu are distinct identity-only accepts; saved Maître Makatsu is searchable. Native counts43 accepts/245 holds/55 reviewed/233 unreviewed;73 alternate terms on26 identities. Full primary/source/entity reviews preserve Wikidata-only numeric authority, original complete holds/null dispositions and literal HTTP P856 versus observed HTTPS requests. All285 other decisions,893 approved/draft files and44 overrides remain unchanged. No rule or edition transfer. Independent actual getters and both public search endpoints pass. See [the complete checkpoint](research/coverage/sep26-amigo-and-quick-progress.md).

Player-count controls now measure44×44 targets. Independent preceding-build audit confirmed room at320px and reproduced an offscreen12-player Quick result after the first touch Pick. A post-commit geometry check brings offscreen Quick/Instant winner text into view once per draw with smooth normal/auto reduced motion and no focus/RNG/outcome/deadline change. Original native evidence is preserved. Compiled browser/visual results and final custody are recorded in the checkpoint after their gates finish.

This is a **provisional integration handoff**. Original shared browser selection:61 passed/5 failed out of66. Three new focus assertions used the button's pre-result accessible name and are corrected; two WebKit reduced-motion large-group cases timed out under concurrent shared changes, with missing trace files/ENOENT. Corrected checks moved to an isolated checkout. Ignored source-cache byte checks are conditional on local presence for clean-clone CI; metadata/provenance assertions remain unconditional. Clean-clone isolation passed120 unit tests/11 files with caches absent. The merge task will run final integration gates. No final all-pass browser claim is made; exact original results and frozen build/source seal are in the complete checkpoint. Independent frozen Chromium foreground and directory/no-JS checks passed; root read the completed memo. Hidden lifecycle remains inconclusive.

**Full verify passed**126 checked files/zero diagnostics, lint,127 unit tests/12 files,958 pages/19,155 links and content/SEO/static/security/cache audits. Build12.44s; gzip111,743 initial JS /1,161 Balloon /128,603 basic home bytes (+122/0/+124 versus preceding build). **Release matrix passed** empty preview/production, isolated fixtures, disabled Balloon and missing configuration rejection. Original strict tuple-type diagnostic in the new unit test is preserved; adding const tuple typing resolves it before the successful verification. Seven tests/three checked files belong to pre-existing unrelated growth work.

Corrected isolated focus/unchanged-scroll checks passed3/3 in allthree browser engines; the two WebKit stability reruns remain pending. The intervening11-file homepage/theme commit d21eb25 is separately owned and excluded from this checkpoint's twelve-file stage scope. The remaining sections record historical checkpoints. Full compendium/perpetual goal active; no deployment or ranking/revenue/physical-device claims.

## Brass identity and remembered-count correction — 2026-09-26 UTC

Current local scope: **4,996 identities /893 approved editions /887 sourced /4,109 pending /361 portable criteria /60 house prompts**. Brass452264 is accepted as an identity only, with Wikidata-only numeric authority and saved native spelling. En Route418683 remains held after complete root review of its primary406454 and2024/2025 conflict. Full prior holds, null dispositions, all286 other decisions and both false rule flags are preserved; four cached sources appended. All893 approved/draft files,44 overrides, discovery snapshots and rule assignments remain unchanged. Independent actual runtime/public projections pass. See [the complete checkpoint](research/coverage/sep26-brass-and-remembered-count-progress.md).

An independent native-time audit reproduced two focused early25→12 count overwrites in WebKit. Removing passive count synchronization and batching count with actual roster changes corrected **15/15 identical diagnostics** in allthree engines. Every first editable restored count was12/12; every25 edit persisted. Original failures remain sealed. Source/controls/reveal timing/randomness/CSS limits are recorded in the checkpoint.

**Full verify passed**124 checked files/zero diagnostics, lint,126 unit tests/12 files,958 pages/19,152 links, content/SEO/canonical/structured-data/sitemap/private-content/indexing/security/cache/static-answer audits. Build18.68s; gzip111,621 initial JS /1,161 Balloon /128,479 basic home bytes (+5/0/+5 versus preceding build). **Release matrix passed** empty preview/production, isolated fixtures, disabled Balloon and missing configuration rejection. Final check/lint again passed124 files/zero diagnostics. Seven tests/three checked files are pre-existing unrelated growth work.

Selected browser gates passed **45/45 /169.564s** (24new Brass/first-edit cases plus21existing count/pointer/touch/fallback/scrolling) and separately **36/36 /331.874s** (complete result continuity) across Chromium/Firefox/WebKit at320/1280, normal/reduced where relevant. **81 selected cases across two runs**, zero retries/skips/flaky/failures; no uninterrupted81-case/full-suite claim. All root sessions closed. Logs/results artifacts/sep26-brass-and-count-{verify,release,browser,continuity-browser,final-check,final-lint}*.

Frozen visual audit:8 contexts/14 states/28 opened PNGs in normal Chromium at320/1280. Pending search/disclosures/filters, distinct existing Brass article→Back, held exclusion, no-JS B2 and cold count edits pass. Initial broad-search expectation3 missed legitimate Quatre Bras30022; two original harness negatives are preserved, corrected catalog continuation passes with exact frozen-index expectations. No app change from that correction. No overflow/errors/external requests; all owned handles closed. Fields/steppers≥44px high, count34px and steppers40px wide; no44×44 claim. Mobile expanded BGG action requires ordinary scrolling for its last6px. Root read full report/opened five representative captures. Report `2d00228abfbdde34e386c6d773a346119e648ba4be353c6806656a955d5cdcf8`, manifest `87fc1baa3e8e0f6f338d59fe3357118a301520ad0ab38b1546c30ad3a83c037c`.

Root rehashed987 compiled files/1,045 exact source copies/1,228 evidence members plus893 unchanged approved/draft files and44 overrides. Build seal `fa6b04c7ab48da156b8ea0f025f2d816453c087c491b8c36454cce780ea3e579`; final report `508a7ca545f641ff8049b2e7b58d11550e5c47189a3c93be1325a98d6b1cb4ea`, seal `b3713d370301bca509501ac500e929833f9052cf2a20bd22eeadc03d60d23ef6`, evidence manifest `a2689538cc858ca713a1ba2e3ed169f5bd33e9b87618817a16fe4285c13a6cd1`. Search index355,844 raw/90,246 gzip bytes (+100/+52); checked-rule index unchanged. Exactly ten owned files are checked/staged separately. Five unrelated files and4173/PID24812 untouched. Separate Amigo intake remains unapproved/unenrolled. Full compendium/perpetual goal active; no deployment, ranking/revenue/field-performance/physical-device claim.

## Picker result continuity — 2026-09-26 UTC

Coverage remains **4,995 identities /893 approved editions /887 sourced /4,108 pending /361 portable criteria /60 house prompts**. Completed winner/outcome/live paragraph and artwork now persist when selecting the next reveal or opening/closing an unchanged list. The captured draw mode owns scene/loading/boundary/busy presentation; next-draw controls own preferences/fallback. Actual roster edits and Forget clear stale results; new real draws use the latest supported method. No randomness/deadline/CSS/artwork change. See [the complete checkpoint](research/coverage/sep26-picker-result-continuity-progress.md).

**Full verify passed**122 checked files/zero diagnostics, lint,126 unit tests/12 files,958 pages/19,151 links and metadata/canonical/structured-data/sitemap/private-content/indexing/security/cache audits. Build21.39s; gzip111,616 initial JS /1,161 Balloon /128,474 basic home bytes (−10/0/−9 versusca342e0). **Release matrix passed** empty preview/production, isolated fixtures, disabled Balloon and missing configuration rejection. Seven tests/three checked files are unrelated pre-existing growth work. Logs `artifacts/sep26-result-continuity-{verify,release}.log`.

Initial selected browser gate **52/57 /499.431s**:35/36 new and17/21 existing cases passed. Three old tests expected Pick-a-player after preference change; now Pick-again remains correct. One WebKit remembered-count case typed before checking settled count; final12/216ms trace does not prove native lost-edit causation. Test now asserts restored roster/count12 before25. One new WebKit1280 normal case reached its second native reveal after repeated node checks but exhausted30s; the whole-test allowance is60s, with unchanged application deadlines. **Corrected affected gate18/18 /197.763s**, no retries/skips/flaky outcomes, allthree engines at320/1280 and normal/reduced motion where relevant. All36 new and21 existing selected cases covered across runs; no clean uninterrupted57-case/full-suite pass claimed. Final test-only check/lint passed122 files/zero diagnostics. Logs/results `artifacts/sep26-result-continuity-{browser,corrected-browser,final-check,final-lint}*`; original negatives preserved.

Native frozen Chromium visual audit: four contexts/12states/24PNGs, all24 opened by independent reviewer; root read complete memo/opened four representatives. Same Card answer/scene/winner/live nodes across disclosures/preferences; actual next Coin draw; native keyboard focus retained. Fields≥44px/no measured clipping, no overflow/page/console/HTTP errors/external requests. All owned handles closed. Desktop header answer can be above viewport after existing scene scrolling; retained mobile result moves below opened list. No universal viewport/geometry/animation-frame/assistive-speech/physical-device claim. Report `b77087657631188094b2e1a9be76d4a7c159f78f16b6e46aabf19eb483727a36`, manifest `ef8f75d74e1292cc2ac0c2b620349752b104b51264039a87f2cd9faad235260e`.

Root rehashed987 compiled files,1,014 original source copies and complete effective1,014 final inputs (1,012 unchanged plus two corrected tests); all893 catalog/identity inputs unchanged. Initial freeze helper's non-JSON counting negative remains preserved. Frozen build seal `95ce2cb74feece36c6f9dec88f9976356c67a7c6d82bc13c138a49a90d48f3d0`; effective source manifest `045a403f02c0cd7e58704cb09cd40f735e83590d3c78754ab3005b48bc6f5a1d`; final rehash report `12106a6832459b41d895ffdadc922061ac8c702f1c5c29128631beac39396982`, seal `caee47d7c2670b6a00f2aafdabba7b929df545ec46091679a2aee230155841af`,86-member evidence manifest `279b9abfe3d9c46e203f213bbc735a7b4468df8097cb9272ebcf98d08806e386`. Exact eight owned files are staged separately; five unrelated files untouched. All owned servers closed, unrelated4173/PID24812 untouched. Brass semantic identity proposal received root original-source review and remains unenrolled; EnRoute conflict remains held/partially read by root. Full compendium/perpetual goal remain active. Nothing deployed.

## Two Russian manuals and native checked-rule search — 2026-09-26 UTC

Current local scope: **4,995 identities /893 approved editions /887 sourced /4,108 pending /361 portable criteria /60 house prompts**. Spectacular and Yami add exact reviewed Russian manual summaries, bounded to simultaneous multiplayer phases and the opening training mission respectively. Root and an independent reviewer read all20 original pages. Great Library remains pending a final source; Yami/Kakapo remain separate. All891 preceding records,42 preceding mappings and native decisions remain unchanged. Validated native names now reach assigned checked editions on home and public library search without altering approved aliases or assigning rules. See [the complete checkpoint](research/coverage/sep26-two-manual-rule-search-progress.md).

**Full verify passed:**121 checked files/zero diagnostics, lint,126 unit tests/12 files,958 pages/19,151 internal links, canonical/metadata/structured-data/sitemap parity and indexing/private-content/security/cache audits. Build22.85s; gzip111,626 initial JS /1,161 Balloon /128,483 basic home bytes unchanged. **Release matrix passed** empty preview/production, isolated fixtures, disabled Balloon and missing configuration rejection. No deployment. Seven unit tests/three checked files are unrelated pre-existing growth work.

Initial selected browser run **59/60 /4.1m**; all48 existing cases passed. One new WebKit1280 test reached its final clear assertion but exhausted the30-second aggregate limit after covering both surfaces. Original test/trace/context/screenshot remain preserved. Same assertions split by surface passed **18/18 /133.848s** across Chromium/Firefox/WebKit at320/1280, with no retries/skips or timeout/application change. Final test-only check/lint passed121 files/zero diagnostics. No uninterrupted60-case/full-suite pass claimed. The npm shim stripped intended grep/reporter options; actual initial run covered allthree selected files and produced no JSON report. Logs: `artifacts/sep26-native-rules-{final-verify,final-release,final-browser,corrected-browser,corrected-check,corrected-lint}.log`.

Visual review retains six before states/12 PNGs and18 after states/36 PNGs across allthree engines/both widths. Independent reviewer opened all36 after images; root opened allsix before viewports and two after home viewports. Native Stars query now produces the exact checked target on home/library/directory, with readable query/results and no visible clipping/overlap or recorded overflow. Separate no-JavaScript Chromium review captured eight article/shelf states/16 PNGs; root opened allfour full articles and four shelf viewports. Source folders/filenames/plain locators and optional house choices remain usable. All owned handles closed; unrelated4173/PID24812 untouched. Static captures do not establish focus/Back/reload/animation behavior; browser assertions cover native Back. No new animation change.

Root rehashed **987 compiled members**,1,013 original source copies,1,012 current matches plus one corrected test,132 intake members,10 independent manual-review members, all891 previous rules and42 preceding mapping objects. Frozen build seal `a7b53b0d558a4e1e62246054366dbac834fae048b5fa4e88c649cd6f01d1138b`; final rehash report `080aa0e69789741df13549210a40b1bc1182ac93c138393d3dcb3cc3bd4a4973`, seal `890110c69e862485fc250501ea345d36f32e39bd5649f018c300abe304b0f5db`,106-member evidence manifest `6a40ffcd4931ec452742842b811e2e4ecc7aac713f21898c8a8d65bf1b0e27a7`. Original source seal predates the sole test split; its explicit corrected snapshot is bound separately. Using identical Node level9 gzip, lazy rule lookup increases1,588 raw/1,144 gzip bytes; directory search increases261 raw/79 gzip. No field-performance/SEO ranking/revenue/physical-device result claimed. Exactly18 owned files are staged separately; five unrelated growth files remain untouched. The full compendium and perpetual goal remain unfinished.

## Three native identities and home focus visibility — 2026-09-26 UTC

Current local scope: **4,995 identities /891 approved editions /885 sourced /4,110 pending /361 portable criteria /60 house prompts**. Root reviewed Spectacular, The Great Library and Yami as distinct pending identities using complete saved entities, six primary bodies and literal numeric publisher hrefs. Yami and Kakapo remain separate. The native pool has39 accepts/249 holds/50 reviewed/238 unreviewed and71 alternate names across24 identities. All285 other decisions,42 overrides and891 catalog files remain unchanged. The home rule-result focus ring now sits inside its clipping list. See [the complete checkpoint](research/coverage/sep26-three-native-focus-progress.md) for custody, exact bindings, tests and limits.

Full verification passed119 checked files with zero diagnostics, lint,124 unit tests/12 files,956 pages and19,125 internal links plus SEO/indexing/private-data audits. Selected browser run43/45; two Firefox assertions incorrectly retained pointer modality after programmatic focus. Native Tab test correction passed6/6 affected cases across all engines; no clean45/45 single run is claimed. Final test-only type/lint gates passed. Independent visual audit confirms18/18 complete rings; root inspected12 pending-directory viewports. Exact final build/source/evidence are sealed, initial failures and startup timeout retained. Initial JS stays111626B; conservative home gzip128483B (+13B). Manual discovery is prepared for next review; Great Library's listed manual is explicitly nonfinal. No deployment occurred.

## Akarios and home interaction continuity — 2026-09-26 UTC

Current local scope: **4,992 identities /891 approved editions /885 sourced /4,107 pending /361 portable criteria /60 house prompts**. Stars of Akarios adds a separately reviewed English summary of the exact Russian main manual, scoped to1–4 players and combat/world pilot actions. Root and an independent reviewer read all52 complete pages, checked publisher→folder→literal filename→original PDF custody and rehashed161 sealed source members plus49 preceding members. The exact approved revision is `c306ccb03d3d76385e5bf2fc635c4397b3348769d803133149f23623d1016aec`. All890 preceding catalog files,41 preceding overrides, native decisions/holds/transfer flags and portable eligibility remain unchanged. Scenario-book/FAQ/errata/expansion and English-version limits remain explicit. See [the complete checkpoint](research/coverage/sep26-akarios-home-continuity-progress.md).

Home keyboard draws retain the same native action button through reveal/result, with guarded busy activation and completion preserving native Tab/pointer destinations. Home lookup saves bounded query fragments and restores reload, native Back, bookmarks and hash changes with lazy search/revision cancellation. Completed unchanged persisted restoration preserves the focused result node. A clean **simulated** persisted-pageshow baseline reproduced link deletion; native Back had `persisted=false`, so no actual native BFCache defect is claimed. Source, baseline and independent review artifacts are recorded in the checkpoint.

**Final full verify passed**118 checked files/zero diagnostics, lint, **124 unit tests/12 files**, content validation, **956 pages/19,122 internal links**, metadata/schema/canonical/sitemap parity, private-content exclusion, security/cache and budgets. Build10.42s; gzip **111,626 initial JS/1,161 Balloon/128,470 basic home bytes** (+278/+298 initial/home versus141de8d). Log `artifacts/sep26-akarios-home-final-verify.log`. Initial full verify/release logs remain preserved. Initial release matrix passed preview/production/isolated fixtures/disabled Balloon/missing configuration rejection **before** the small persisted-query follow-up; no exact-final-source release rerun is claimed. Seven unit tests/three checked files are pre-existing unrelated growth work, excluded from owned scope.

Initial selected browser gate: **49/51 passed/3.5m**, with all39 cases outside keyboard-draw tests passing. Two normal-motion WebKit tests mixed native keyboard focus assertions with mouse activation, and the1280 click arrived1567.372ms after the first Enter (Quick timer1242ms), allowing a valid later draw. A separate four-context native WebKit diagnostic confirmed keyboard-only focus persists, while a busy pointer immediately moves focus to MAIN and completion preserves it. Zero errors/external requests; same button in allfour, own sessions closed. Original traces, context/PNG negatives and test bytes remain unchanged. Test correction keeps11 cases, checks uninterrupted native-time keyboard completion before a separate paused-clock busy guard and preserves each engine's native pointer destination. Diagnostic report `454b3d39c35bb1c2427278bafc8067a24df581b54adf61514561993f9385a7b5`; measurements `5342a0a5c905aa21f32bff822e30f49ab71db91c7062c96f0431951f12cc8c6f`; manifest `5420a31c5359f44272255d59968c1f4d1296e4e39cd5d6384fbab6c71280ba32`. Logs/results `artifacts/sep26-akarios-home-browser*` and `sep26-webkit-busy-pointer-diagnostic/`.

**Corrected keyboard gate passed12/12/1.9m** across Chromium, Firefox and WebKit at320/1280 with normal/reduced motion. Final test-only check passed118 files/zero diagnostics and lint. Logs/results `artifacts/sep26-akarios-home-corrected-keyboard-browser*`, `sep26-akarios-home-final-{check,lint}.log`. All root sessions closed.

Frozen Chromium visual review completed **eight flows/all48 resulting PNGs opened**, including20 excluded partial-attempt images, two honestly labeled late320 result frames and two exact pixel crops. Native keyboard focus/3px outline, restored ordered Azul results/scroll and Russian search→complete Akarios article passed at both widths; simulated unchanged persisted restoration preserved its focused native link. No overflow/clipping/errors/external requests; all own handles closed, no app/RNG/animation override or rebuild. Actual1280 busy frame is visible; no320 busy screenshot or actual native BFCache result is claimed. The uppercase-heading expectation failure remains historical, excluded from final measurements. Root opened four representative captures and read the complete report.

Root independently rehashed985 compiled files/10,869,795 bytes,1,001 source inputs equal current and1,069 evidence members, plus161 approved intake members, preceding984 site/27 source/62 evidence members, seven flat bindings and six earlier seal/report/manifest bindings. Original root helper's old previous-binding-format assumption stopped at an absent path; its negative log remains preserved. Corrected helper passed. Report `7a68fa98f899b6c04ae59616935af64de71accf90af418b9dd93430af6efb086`; seal `a27bfcbd4d25b20d8fcff1039b6f5878ea7c9c62c4502d2bce71ad219878507f`; evidence manifest `d5d2c1ba7563305460e8c013906f3b5df91c7ea34eedc395a95b4265fa665ca2`. Final search355,165 raw/89,956 gzip bytes (**+156/+62**); stable final test SHA `af1c260aa6ee750edb332b85ee9de9313a0f76aa266d89e862062089655c451b`. Full hashes/bindings and limits are in the checkpoint and `artifacts/sep26-akarios-home-final-visual/`, `sep26-akarios-home-root-rehash*`.

Final unstaged and `--staged` scope passed exactly16 owned files and all prior-content/input/pool invariants, with `stagedVerified:true`. Logs `artifacts/sep26-akarios-home-{final-scope,pre-doc-staged-scope,staged-scope}.log`; the final documentation-only restage is checked again before commit. Five unrelated marketing/growth files remain untouched and unstaged. No uninterrupted51-case/full-suite, physical-device, field-performance, ranking, revenue or deployment result is claimed. The full compendium and perpetual goal remain active; nothing deployed.

## Native-name search and three CrowD-linked manuals — 2026-09-26 UTC

Current local scope: **4,992 identities /890 approved editions /884 sourced /4,108 pending /361 portable criteria /60 house prompts**. Aqua Garden, Dino Garden and Harrow County add separately reviewed exact English manual answers; Stars remains pending. Root opened all74 complete rendered pages, checked publisher→folder→verbatim filename→original PDF custody and rehashed242 sealed intake members,49 preceding manual-source members, seven bound inputs and887 preceding rules. Three explicit overrides and exact approval revisions are recorded in [the complete checkpoint](research/coverage/sep26-native-search-three-manual-progress.md). Original proposals, identity decisions/holds/transfer flags and38 preceding overrides remain unchanged. Aqua/Dino open choice, different Lucy movement and track order, Harrow tutorial/full-game/Hester/solo and changing token priority remain distinct. Stable folder citations supply filenames and honest PDF/printed locators without invented viewer/page URLs.

Public directory search now uses **66 validated alternate titles across21 accepted native identities**, deduped with the same Unicode/punctuation normalization as the client. Each title binds to its saved entity label or nondeprecated P1476 statement. Held names/private provenance are excluded. Search names never participate in rule matching; inventory/coverage keeps its strict shape. Exact native snapshot/decision content hashes invalidate the private search-name cache. Display titles, numeric IDs, all36 accepted/252 held decisions and portable eligibility remain unchanged.

**Full verify passed**117 checked files/zero diagnostics, lint, **124 unit tests /12 files**, content validation, **955 pages /19,109 internal links**, metadata/schema/canonical/sitemap parity, private-content exclusion, security/cache and budgets. Build19.45s; gzip **111,348 initial JavaScript /1,161 Balloon /128,172 basic home bytes** (-11/-8 initial/home versus27cdd1f). **Release matrix passed** empty preview/production, isolated synthetic fixtures, disabled Balloon and missing configuration rejection. No fixture deployed. Seven tests/three checked files are pre-existing unrelated growth work, excluded from owned scope. Logs: `artifacts/sep26-native-search-three-manual-{verify,release}.log`.

Selected browser gate passed **48/48 /2.6m** across Chromium, Firefox and WebKit at320/1280:30 existing directory/article cases, six native alternate-search/filter/held-exclusion cases and12 CrowD mixed sourced/pending/native-keyboard/no-JavaScript shelf/article cases. Exact source-folder links, filename guidance, plaintext PDF locators, no unsupported PDF shortcut, no selection-tie section, optional house methods and no horizontal overflow passed. Root sessions closed. No full-suite/physical-device result claimed. Logs/results: `artifacts/sep26-native-search-three-manual-browser*`.

Independent source proof SHA-256 `9c2286761bf0b83d342170a09e154dd5599cd9a7ac42c788b7d4bc855a85879a`; final exact-draft proof `c7c63549e2f74fa30d333390fb0d8c583e509de1280c3d547dbef52a8c7131c4`; intake manifest `39ea700f62a8bafa0fad81f7d3c07cba72998d42d09d3e6cebdfc822d8988dd7`. Root read both independent memos and rehashed their proofs before approval.

Final frozen visual audit passed **14 states /28 PNGs**: native Russian searches and complete articles at320/1280 in Chromium, all opened, no overflow/errors/external requests and all own sessions closed. Root opened four additional representative complete captures and independently rehashed984 compiled files,27 exact source copies and62 evidence members; six prior seals remain unchanged. Report SHA-256 `6003e7eb0e4c77a9d2da218f8a7925d2b220cdba479109c1b9d6a38f9a04adc7`; evidence manifest `23933ee82eae79a8794c08bee86f557799ff5aef603576b22c387a2e4f09b52d`; seal `25a26323fcbe862391df3a30ae65c15d5e044a9db3a3a72dc383b467f2e28507`. Final search index355,009 raw/89,894 gzip bytes, **+1,406/+1,616 bytes** versus exact preceding compiled bytes. This includes66 accepted native terms and three new article labels/slugs. Evidence: `artifacts/sep26-native-search-three-manual-visual/` and `sep26-native-search-three-manual-root-rehash.*`; full manifest hashes and limits are in the checkpoint.

Final `--staged` scope passed with `stagedVerified:true`, exactly20 owned implementation/content/test/documentation files and all original rule/input/override invariants. Log: `artifacts/sep26-native-search-three-manual-staged-scope.log`. Five unrelated marketing/growth files remain untouched and unstaged. Root rehashed161 sealed Akarios intake members; its52-page Russian source remains separately unapproved, awaiting complete root review. The full compendium, perpetual goal and commercial outcomes remain unfinished; nothing deployed.

## Four CrowD identities enrolled — 2026-09-26 UTC

Current local scope: **4,992 identities /887 approved editions /881 sourced /4,111 pending /361 portable criteria /60 house prompts**. Root reviewed and enrolled Stars of Akarios, Aqua Garden, Harrow County and Dino Garden as pending identities; Octex remains a reviewed musician misassociation hold and Sky remains unbound. Exact native titles/P2339/P123/domain chain, age/year/player-count/edition limits and all complete prior holds remain preserved. Three exact primary hrefs remain distinct from Dino's semantic corroboration/Wikidata-only numeric authority. Dino's unbound collection image href is explicitly retained without a title binding; its original sealed proposal is unchanged. Native review:36 accepts/252 holds/47 reviewed/241 unreviewed. See [the complete identity checkpoint](research/coverage/sep26-crowd-four-identities-progress.md).

Root rehashed88 original intake members, seven exact bound inputs per bundle and887 approved files, prepared an ignored clone and ran real-module collision/projection checks before guarded application. Exactly five decisions changed and283 remained unchanged; old source entries, snapshots, identity overrides, catalog/approved revisions, app code and dependencies remain unchanged. Applied decisions:1,080,458 CRLF bytes/SHA-256 `26bcf9e4e0545c4e0774d87f3c993cbdbc9bfe28c81ca17b64e8abd91293f565`. The first ignored helper stopped on absent optional byte metadata before mutation; the corrected preparation/preflight/application and Python native validators passed, preserving the original failed log. Evidence: `artifacts/sep26-crowd-root-application*`, `sep26-crowd-root-runtime-preflight*`, `sep26-crowd-identities-scope*`.

**Full verify passed**116 checked files/zero diagnostics, lint, **122 unit tests /12 files**, content validation, **952 pages /19,070 internal links**, metadata/schema/canonical/sitemap, private-content exclusion, security/cache and budgets. Build20.77s; gzip **111,359 initial JavaScript /1,161 Balloon /128,180 basic home bytes**, unchanged from8021cfc. After the new focused browser file, final check/lint passed117 files/zero diagnostics. Selected browser gate passed **42/42 /2.1m** across Chromium, Firefox and WebKit:30 existing directory/article cases plus12 new cases covering all four names/IDs, pending search/disclosure/links and ordinary no-JavaScript shelves at320/1280. No full-suite, physical-device, new release-gate or deployment result is claimed. Logs: `artifacts/sep26-crowd-four-identities-{verify,browser,final-check,final-lint}*`.

Independent final compiled visual/provenance checks cover all four native searches at both widths and Harrow/Dino no-JavaScript shelves. The original mobile row-border rounding assertion and later ignored selector/CSP-evaluation harness failures remain preserved; native scrolling completes the actual mobile captures without app edits. Exact scope checks retain eight owned staged files; five unrelated untracked files stay untouched and unstaged. Seven unit tests/three checked files belong to that unrelated growth work. Full English manuals were separately discovered through the observed public folders and remain under source review; no new rule is approved by this checkpoint. Worldwide coverage and the perpetual goal remain unfinished.

Visual audit completed **12 requested entry checks /24 PNGs**, plus one preserved original negative PNG. Root opened complete desktop/mobile Harrow captures and rehashed981 compiled files,18 source copies and56 evidence members. All final requested checks show exact pending names/IDs, zero article links, two44px actions and no overflow/external requests; all own sessions closed. Report SHA-256 `66b9592eb61d5e9f0b03f9acd7214f8d9609604e6f7c3e957fa50c4cec17cfd2`; evidence manifest `d3df9e5812f77e743115f1bae21ce93ff6d5840737ca690857539945524bd3a3`; seal `7296c6a3bf3135618c384b4480e9eb9e9fc54347412ab2ab17bcdfce0dfadcfc`. Independent postapplication memo confirms exact five changes/283 unchanged, all prior sources and887 approved rule hashes; root rehashed its proof `8fc570e65480759e2582b37bf557282570c8e03cae4a866d86820aef956497e7`. See the complete checkpoint for compiled/source manifest hashes and preserved harness failures.

## Navigation recovery and CrowD intake — 2026-09-26 UTC

Current scope unchanged: **4,988 identities /887 approved editions /881 sourced /4,107 pending /361 portable criteria /60 house prompts**. Public rules searches restore reload, native Back and bookmarks using query fragments. Multi-page shelves expose identical native pagination above/below games; Privacy has a44px return and accurate fragment/history wording. Frozen final Chromium evidence covers four journeys/26 states/52 PNGs at320/1280, including no-JavaScript links. Root opened complete mobile Privacy and A2 captures, and rehashed baseline/final compiled, source and evidence archives. Three CrowD plus Dino identity proposals are prepared, independently reviewed and not enrolled; Octex/Sky/manual source holds remain bounded. See [the complete checkpoint](research/coverage/sep26-navigation-and-crowd-intake-progress.md).

**Full verify passed**116 checked files/zero diagnostics, lint, **122 unit tests /12 files**, content validation, **952 pages /19,066 internal links**, metadata/schema/canonical/sitemap, private-content exclusion, security/cache and budgets. Build19.74s; gzip **111,359 initial JavaScript /1,161 Balloon /128,180 basic home bytes** (+214/+212 initial/home from6696b83). **Release checks passed** empty preview/production, isolated fixture, disabled Balloon and missing configuration rejection. No fixture was deployed. Seven unit tests and three checked files belong to unrelated untracked growth work, excluded from this checkpoint. Logs: `artifacts/sep26-navigation-recovery-{verify,release}.log`.

Selected browser gate: **117/123 passed /2.6m**, all105 pre-existing cases passed; six new cases reached home then used the wrong heading. Heading-corrected18-case retry and undefined-font-return retry each passed16/18, with Firefox no-JavaScript page-promise hangs/cleanup failures. Separate five-context diagnostic found even an ordinary resolved page promise pending with JavaScript disabled, despite loaded fonts, zero font requests and successful native screenshots; enabled control resolved both. Test-only synchronous font-state plus existing native geometry/link assertions fixed the harness: **18/18 passed /1.6m** across Chromium, Firefox and WebKit. Original negatives, traces and three test snapshots remain preserved. No uninterrupted123-case/full-suite pass is claimed. Final test-only check/lint passed116 files with zero diagnostics. Logs/results: `artifacts/sep26-navigation-recovery-{browser,corrected-browser,font-corrected-browser,nojs-corrected-browser}*`, `sep26-navigation-recovery-nojs-final-{check,lint}.log`.

A separate eight-context Chromium diagnostic reproduces an early330px DOM width before stylesheet application; styled load/frame/native captures fit320px. Its24 PNGs do not establish a visible flash or persistent defect, and the older sample remains preserved. Root read both diagnostic reports and rehashed their evidence. All browser/context/server sessions are closed. Exact scope checks verify unchanged catalog/discovery/native inputs and exactly eleven staged owned files; five unrelated untracked files remain unstaged. No deployment, worldwide coverage, field-performance, ranking or revenue result is claimed. The perpetual goal remains active.

## Final Titan and directory search continuity — 2026-09-26 UTC

Current scope: **4,988 identities /887 approved editions /881 sourced identities /4,107 pending**, with **361 portable criteria /60 house prompts** unchanged. Final Titan has a reviewed English summary of the Russian GaGa ©2026 manual, bounded to main competitive play with3–5 players. Its literal Russian alias, highest-speed opening, successive bench-speed comparisons/lots, youngest setup draft, later arena/rotation rules and card-priority reminder have exact sources. Root opened21 complete manual pages and rehashed93 original identity/manual intake members; an independent reviewer checked the same full source context. One native identity was accepted and three retained bounded holds, preserving full prior decisions and all other entries. Numeric provenance remains Wikidata-only. The research draft and approved catalog revision remain separate. See [the complete checkpoint](research/coverage/sep26-final-titan-directory-history-progress.md).

Directory queries and filters now survive reload, native Back and bookmarked navigation together. Input/filter changes replace the current history entry, preserving its state; clearing resets the filter. Unicode punctuation, literal legacy plus signs, unrelated anchors, blocked history writes and static browsing remain usable. Eight baseline and eight final independent compiled Chromium journeys at320/1280 preserve exact result order/IDs/hrefs/counts after the fix, including80 capped pending results and filtered zero results. Each audit retains34 PNGs. Root opened representative captures and independently rehashed the final980 compiled files, five source copies and45 evidence members. The later Final Titan audit uses the current952-page build; its separate four captures cover320/1280 with JavaScript bookmarked Russian search→article→Back and without JavaScript direct articles, with16 PNGs. No observed overflow, browser errors or external requests occurred.

Final **`npm run verify` passed**: **114 checked files**, zero errors/warnings/hints, lint, **122 unit tests /12 files**, content validation, **952 preview pages /19,017 internal links**, metadata/schema/canonical/sitemap parity, private-content exclusion, security/cache and static budgets. Build **16.60s**; conservative gzip **111,145 initial JavaScript /1,161 Balloon /127,968 basic home bytes**. Initial JavaScript grew230 gzip bytes and home231 from saved `58ff461`; no dependencies/fonts/media were added. Seven unit tests belong to pre-existing unrelated untracked growth work and are excluded from this checkpoint. The initial new-catalog verify found three dated count assertions (**119/122 passed**); corrected assertions passed the full rerun. Evidence: `artifacts/sep26-final-titan-history-{final-verify,corrected-verify}.log`.

The selected final browser gate passed **69/75 cases /2.0m**, including all60 directory/history/search/startup cases and all no-JavaScript cases across Chromium, Firefox and WebKit. Six new Final Titan assertions incorrectly expected the house-fallback heading instead of the rendered official tie heading; the additional page14 link also uses the source-list label. Tests alone were corrected. **All15 corrected article cases passed /59.8s** across the three engines, including Russian bookmark/native Back. No uninterrupted75-case or full-suite pass is claimed. Original failure screenshots/traces/logs and the original test snapshot are retained separately from the corrected results. Post-correction check/lint passed. Evidence: `artifacts/sep26-final-titan-history-final-browser*`, `sep26-final-titan-article-assertions-before-correction.spec.ts`, `sep26-final-titan-article-corrected-{check,lint,browser}*`. The earlier search-only60/60 gate is separately retained.

**`npm run test:release` passed** preview-empty, production-empty, isolated synthetic content, disabled mode and missing-domain/contact/hosting rejection. No fixture was deployed. Exact scope validation passed all prior-record/override/native-input comparisons,93 original intake hashes, final approval revision, runtime counts and Northgard pending. Its first ignored helper exceeded Git's output buffer; its next helper attempted to parse `.gitkeep`; both failures remain preserved and the corrected read-only check passed without app changes. Logs: `artifacts/sep26-final-titan-history-{release,final-scope,corrected-scope,final-valid-scope}.log`.

Root independently rehashed the final article audit's981 compiled files, nine source snapshots and32 evidence members; scripted/no-JavaScript article PNGs match exactly at both widths. Evidence: `artifacts/sep26-final-titan-article-root-rehash.{py,log}`. Browser/context/server sessions are closed.

Final exact scope also passed with `--staged`, verifying exactly16 owned files and the same source/content invariants; `stagedVerified=true`. Log: `artifacts/sep26-final-titan-history-final-staged-scope.log`. Unrelated untracked files were not staged.

The initial consented analytics configuration excludes search fragments and campaign/private query parameters; a new unit case verifies that boundary and local fragment retention. Downloaded tag behavior and remote Google stream settings remain unverified. Before deployment, verify history-based enhanced page views are disabled and inspect consented synthetic fragment traffic as recorded in the checkpoint. Five unrelated untracked growth/marketing files remain untouched. No deployment, physical-device, manual screen-reader, field-performance, ranking, revenue or billing result is claimed. Worldwide coverage remains unfinished; the perpetual goal remains active.

## Two enrolled manuals, readable rosters and visible loading — 2026-09-26 UTC

Current scope: **4,987 identities /886 approved editions /880 sourced identities /4,107 pending**; **361 portable criteria /60 house prompts**. ImmunoWars and Townsfolk Tussle have exact source-reviewed answers with two explicit identity overrides. Root opened21 complete manual pages and rehashed72 frozen intake files plus41 prior source files. A second reviewer checked the same renders and both complete extracted texts. ImmunoWars' startup hand draw and unspecified first normal-turn player remain explicit. Townsfolk is labeled an English summary of the German standard base game; Village/combat directions and later rotation remain distinct. ImmunoWars PDF14 has no visible printed numeral: PDF page14 remains cited, printedPages14 was removed and approval recalculated before the final build. Original proposals/history remain preserved. Neither answer joins the portable mix. Northgard stays pending: the inspected primary path exposed no manual. Five bounded GETs included one exact observed HTTP redirect, with no retries. Native decisions/snapshot and unresolved identity metadata are unchanged. See [the complete source and UX checkpoint](research/coverage/sep26-enrolled-manuals-readable-roster-progress.md).

Compact names now have wider bounded desktop rows and one mobile column; four-player mobile fields use two columns through650px. Complete names, existing fonts, native textareas and ordinary page scrolling remain. The12-name stress roster grows82px at320 and208px at390; desktop shrinks408→234px. The final50-player ordinary mobile fixture has2951px of roster/document4039px; all50 names/IDs remain accessible. Original alternatives/42 PNGs remain in `artifacts/sep26-roster-readability-audit/`. Cold-chunk baseline captures put centered feedback below the viewport; a CSS-only prototype then exposed overlap with the sticky result. Final feedback sits near the start, and draw scrolling measures sticky-result height to leave room beneath it. Selection/RNG/parent-deadline/artwork timing are unchanged. Baseline/prototype/overlap evidence remains in `artifacts/sep26-loading-feedback/`.

Final **`npm run verify` passed**: **113 files**, zero diagnostics, lint, **116 unit tests /12 files**, content validation, **951 preview pages /19,003 internal links**, metadata/schema/canonical/sitemap, private-content exclusion, security/cache and static budgets. Build **9.54s**; gzip **110,915 initial JavaScript /1,161 Balloon /127,737 basic home bytes**. Initial JavaScript grew62 gzip bytes and basic home89 bytes from saved `ef52c5e`; no dependencies/fonts/media were added. Earlier pre-locator-correction verify is separately retained. Logs: `artifacts/sep26-enrolled-manuals-roster-{verify,final-verify}.log`. Seven unit tests belong to unrelated pre-existing untracked growth work and remain excluded from this checkpoint. Post-test-correction check/lint logs are `sep26-enrolled-roster-corrected-{check,lint}.log`.

The selected browser run was **294 cases /three engines /three workers:285 passed /nine failed /8.7m**. All **273 non-roster cases passed**: full150-case Picker file,102 scene/Spinner transitions,12 feedback cases and nine article/search/no-JavaScript cases. Three new roster failures assumed one line for320px four-player names; six assumed WebKit pointer mouseup preserves select-on-focus. Original test/screenshots/traces are retained. The correction checks whole-word fit, permits natural wrapping, verifies native focus selection, then uses a real click and standard keyboard shortcut to edit. No app edit was needed. Corrected **21/21 roster cases passed /1.2m** at320/390/1280 with4/12 and mobile50 players, complete Unicode values,44px fields, unclipped content, native editing/Enter blur and no overflow/inner scroll. **No uninterrupted294-case pass is claimed.** Logs/results: `artifacts/sep26-enrolled-manuals-roster-browser*`, `sep26-first-roster-test.spec.ts`, and separate `sep26-enrolled-roster-corrected-browser*`.

Saved-result aggregation proves **all102 loading transitions have exactly zero scene/control/label/scroll delta**, all96 applicable held native Preferences presses succeed, all12 feedback text ranges remain visible below sticky results, and all21 corrected roster reports satisfy bounds/editing checks. Evidence: `artifacts/sep26-enrolled-roster-results.{py,json,log}`. **`npm run test:release` passed** preview-empty/production-empty/isolated synthetic content/disabled mode and missing-domain/contact/hosting rejection; no fixture content was deployed. Log: `artifacts/sep26-enrolled-manuals-roster-release.log`.

Five independent final compiled Chromium captures use no CSS injection: roster320/12,390/4,320/50 and slow Coin320/24 emoji after real Pick under normal/reduced motion. They preserve exact names/IDs,44px fields, native editing and document scrolling without errors/overflow. The selected winner completes while artwork is blocked; feedback stays below the sticky result. Chunk release changes stage/scene/every label/scroll by exactly0px and preserves the winner. Nineteen PNGs remain in `artifacts/sep26-final-roster-loading-visual/`. Root opened four complete viewport PNGs and rehashed980 frozen files,10 current source snapshots and35 evidence files. Browser/server/contexts closed. This visual audit is Chromium-only; configured-engine evidence comes from the browser gates.

Exact scope validation confirms two approval revisions/overrides, unchanged prior records/native bytes,886/880/4107 coverage,361/60 pools and Northgard pending. Evidence: `artifacts/sep26-enrolled-manuals-roster-final-scope.{ts,log}`. Five unrelated untracked growth files are excluded. No deployment, physical-device, manual screen-reader, field-performance, ranking, revenue or billing result is claimed. Worldwide coverage remains unfinished; the perpetual goal stays active.

## Three primary identities and remaining loading layouts — 2026-09-26 UTC

The active directory lists **4,987 identities**, with **884 approved edition records** covering **878 sourced identities**; **4,109** await rule research. Northgard: Uncharted Lands, ImmunoWars and Townsfolk Tussle have independently reviewed primary semantic identity correspondence. Their numeric IDs remain saved Wikidata-only evidence; each retains its complete prior hold, exact multilingual title, unresolved entities and edition/quantity/year limits. These enrollments approve no starting rule or edition transfer. The **361 portable criteria / 60 house prompts** are unchanged. See [the exact primary review](research/coverage/sep26-three-primary-identities-progress.md).

Root rehashed the frozen 41-file primary bundle, reviewed original product/diary/story context and full saved claims/domain bindings, and compared actual runtime titles and aliases before enrollment. This continuation made **zero new source retrievals**. Native decisions now have **31 accepts / 257 holds**, **38 reviewed / 250 unreviewed**, including seven reviewed holds. The immutable native snapshot remains **3,316,962 bytes / SHA-256 `d13f929adfeb2a0b8a163497b5b45ff3627c7914e7381c86a74152fa575e31b0`**. Decisions are **692,087 CRLF bytes / SHA-256 `c36fcc87bdca413817b7b66a4996ad55ec64bf71d0f44e9b823baabbf3728e88`**. Offline importer validation and coverage passed. Only three decision entries, six new sources and derived counts/method changed; original discovery inputs, identity overrides and all other decisions remain preserved.

Source articles now offer a secondary, conditional link to the player picker after source actions. Its wording applies when the reader's rules call for a random choice and preserves the article's actual opening instruction. Existing house-fallback articles retain their existing picker handoff. The directory continues to use bounded static shelves and a lazy index; these new pending identities do not create empty answer pages.

The independent frozen-build loading audit measured **28 Chromium cases**: all seven remaining lazy methods, two widths and default-four/saved-twelve rosters. Every generic 160px placeholder changed scene height; **22 held Preferences presses missed**. The largest shift was **1,020.25px** for Shell Game at 320px with twelve long/Unicode/duplicate names. The same frozen site had zero observed errors, overflow or external requests. Original harness serialization failures are preserved separately and do not count as application findings. Evidence: ignored `artifacts/sep26-loading-space-audit/report.md`, `corrected/measurements.json` and 112 before/after PNGs.

The lightweight loading scaffold shares the actual grid, geometry-slot and wrapped-label CSS while keeping labels hidden, artwork lazy and RNG/selection unchanged. Its classes cannot satisfy real preview readiness selectors, and it runs no loading animation. Root's first **222-case** runtime gate passed **164 cases in 14.2 minutes**; all **126 pre-existing cases** passed. Its **58 new regression failures** include **46 geometry failures** in Chromium/WebKit caused by the retained generic `place-items:center` declaration and **12 invalid 24-player fixtures** exceeding the existing name limit. Measurements and traces are preserved in `artifacts/sep26-three-primary-loading-browser-results/`; log: `sep26-three-primary-loading-browser.log`. The correction explicitly resets loading alignment and keeps the extended fixture names within 24 graphemes. An independent frozen-copy injection proved zero geometry/scroll change and successful native Preferences presses in four representative cases; root independently checked those raw measurements. Evidence: `artifacts/sep26-loading-space-reset-audit/`.

**All 168 final focused browser cases passed across Chromium, Firefox and WebKit in 3.9 minutes**: 96 new controlled loading replacements, six existing Spinner transitions, all ten preselected-outcome methods, normal/reduced-motion scrolling, empty/invalid rosters, module failures, secure-RNG recovery, Preferences persistence and lazy Balloon loading/share fallback. The read-only static subset used three workers; no live-content fixture cases ran concurrently. All **102 persisted loading measurements show zero geometry/scroll movement**, and every held Preferences press succeeds. The 96 new records also explicitly retain the observed successful click and no-overflow state, including 24-player Dice/Coin, long names, Unicode and duplicate suffixes. Evidence: `artifacts/sep26-three-primary-loading-corrected-browser.log`, its result folder and `sep26-final-scene-loading-summary.json`. These focused results and the preceding 126 unchanged paths are reported separately; an uninterrupted complete-suite pass on the final app is not claimed.

The corrected final app passed **`npm run verify`**: **110 checked files**, zero diagnostics, lint, **116 unit tests / 12 files**, content validation, **949 preview pages / 18,976 internal links**, metadata/schema/canonical/sitemap checks, private-content exclusion, security/cache headers and static budgets. **`npm run test:release`** passed preview/empty production/synthetic content/disabled mode and missing-live-config rejection. Evidence: ignored `artifacts/sep26-three-primary-loading-final-{verify,release}.log`. Seven unit tests and three growth source/test files belong to pre-existing unrelated untracked work and remain outside this checkpoint. Final gzip measurements: **110,853 bytes initial JavaScript**, **1,161 Balloon**, **127,648 basic home**; build **16.04 seconds**. Initial JavaScript grew **209 gzip bytes** relative to the previous saved checkpoint; artwork remains separate lazy chunks. The initial successful verify and its earlier 16.67s/127,638-byte home measurement remain in `sep26-three-primary-loading-verify.log`.

Ten frozen compiled captures cover Wroth at 320/1366, all three new pending disclosures without JavaScript, and actual Shell/Cards/Dice/Coin/Balloon previews/results. They have no observed app/console/HTTP/request errors, overflow, visible label overlap or result/button intersection. Source, handoff and pending action links measure 44px. Seven script-enabled scoped axe checks report no violations but retain incomplete ARIA/contrast review items; no-JS disclosures use direct native interaction/geometry checks. All harness serialization/no-JS scan failures remain documented separately in `artifacts/sep26-independent-final-visual/`. Root opened the complete mobile Wroth and Cards result PNGs. After the correction, an additional **actual compiled Shell320/12** capture has zero measured layout/scroll deltas and a successful held native click; root opened both viewport images together. Evidence: `artifacts/sep26-independent-final-loading-corrected/`. The original Shell loading capture remains a first-proposal baseline, without a final stability claim.

The structural scope check passed after correcting its comparison of older Git-normalized JSON inputs: compare complete parsed content and unchanged Git paths for those inputs, exact bytes/hashes for native snapshots/decisions and overrides. The initial ignored helper's byte-diff allocation failure is preserved in `artifacts/sep26-three-primary-loading-final-scope.log`; no input was modified to satisfy the check. Corrected result: `artifacts/sep26-three-primary-loading-final-scope-corrected.log`.

No deployment, field-performance, physical-device, manual screen-reader, ranking, revenue or billing result is claimed. The perpetual goal remains active and worldwide coverage remains unfinished.

## Three manuals, source viewers and stable Spinner loading — 2026-09-26 UTC

The directory lists **4,984 identities**, with **884 approved edition records** covering **878 sourced identities**; **4,106** await rule research. Ironwood, Wroth and Beyond the Horizon now have independently reviewed answers scoped to their exact language, edition and mode. Root opened **20 complete manual pages**, checked original PDF bytes and publisher anchors, and independently decoded the Wroth viewer's saved asset binding. The intake copies retain draft history; catalog copies bind exact approved revisions and explicit identities. None joins the portable mix, which remains **361 criteria** beside **60 house prompts**. Native decisions/snapshot bytes remain unchanged. See [the complete source checkpoint](research/coverage/sep26-three-manual-progress.md). Worldwide completeness is still unestablished, and the perpetual goal remains active.

Dropbox sharing URLs retain their exact source links and readable cited page lists; a `.pdf` filename on an HTML viewer no longer creates unsupported page-fragment shortcuts. Direct PDFs and reviewed Google exports retain their existing page links. Four earlier viewer articles are covered by explicit independent fixtures for all five source documents, preserving every original URL and citation order. About and the guide now describe reviewed publisher-authored or publisher-maintained sources, including archived/mirrored custody, and distinguish directory listings awaiting rule research.

The full browser run passed **344 of 351 cases** in **28.8 minutes**. Six failures were the two older direct-PDF expectations repeated across the three engines; the seventh was Firefox Preferences during a lazy Spinner replacement. Every original trace/context/screenshot is retained under ignored `artifacts/sep26-three-manual-browser-results/`; log: `artifacts/sep26-three-manual-browser.log`. Trace review found the 160px loading placeholder replaced by the 235px preview while Preferences was clicked. The individual mouse event recipients are not recorded, so the original missed-release explanation remains an inference.

A controlled Firefox reproduction held the real TableReveals download until mouse-down: both **1280px and 320px** cases measured an actual **75px Preferences shift** and failed the unchanged 0.5px geometry limit. The original Preferences case passed an unchanged retry, demonstrating timing dependence without erasing the captured failure. Evidence: ignored `artifacts/sep26-spinner-loading-baseline.log`, its retained results and `sep26-spinner-loading-baseline-geometry.json`. Spinner loading now shares the wheel's responsive width and centered spacing, with a square reserved area. Other methods retain their existing loading layout.

**All 78 final focused browser cases passed across Chromium, Firefox and WebKit in 3.6 minutes**, including all seven formerly failing paths, every board-game/source-page test, six held-pointer loading transitions, normal/reduced-motion scrolling, invalid rosters, module failure, Spinner replay and named seats. All six persisted transition measurements show **zero change** in the button/stage/scene dimensions and document position, stable scroll, and a successful native mouse-up opening Preferences. At 320px the available scene width is 240px, so the wheel correctly retains its 235px cap; the proposed test's assumption that this viewport required a smaller wheel was corrected from the baseline measurements to use actual content width. Evidence: ignored `artifacts/sep26-three-manual-spinner-final-browser.log`, its results and `sep26-spinner-final-geometry-summary.json`. An uninterrupted full-suite pass on the final application is not claimed.

The final application passed **`npm run verify`**: **108 checked files**, zero diagnostics, lint, **116 unit tests / 12 files**, content validation, **949 preview pages / 18,455 internal links**, canonicals, unique metadata, structured data, sitemap parity, private-content exclusion, security/cache headers and static budgets. Seven unit tests and three growth source/test files belong to unrelated pre-existing untracked work, which remains untouched and outside this checkpoint. After finalizing the geometry test's persisted evidence/content-width calculation, the **108-file check** and selected ESLint check passed again. **`npm run test:release`** passed preview/production/fixture/disabled-mode checks and missing-live-config rejection. Evidence: ignored `artifacts/sep26-three-manual-spinner-final-{verify,release}.log`, `sep26-spinner-final-test-check.log` and `sep26-spinner-regression-final-lint.log`.

Eight actual compiled captures cover the three manuals and About at **320px/1366px**; all returned HTTP200, with no measured horizontal overflow or captured browser errors. Root opened the complete four mobile PNGs and desktop Horizon PNG. Evidence: ignored `artifacts/sep26-three-manual-captures/`. Final conservative gzip budgets: initial JavaScript **110,644 bytes**, Balloon **1,161**, basic home **127,188**; final build **22.75 seconds** on this host. These are local checks and measurements, without a new field-performance, physical-device, manual screen-reader, ranking, revenue or billing result. No deployment occurred. The next three native identity leads remain unapproved holds with a rehashed frozen primary-evidence bundle; Northgard and ImmunoWars reassessments are queued for independent approval.

## Prepared search, three discovery identities and mobile focus — 2026-09-26 UTC

The directory now lists **4,984 identities**, with **881 approved edition records** covering **875 sourced identities**; **4,109** await rule research. Ironwood, Wroth and Beyond the Horizon were independently corroborated and enrolled. Exact numeric primary hrefs support Ironwood/Horizon; Wroth retains Wikidata-only numeric provenance beside an explicit Frosted partnership bridge. Complete prior holds, entity/source bindings and metadata/mode conflicts remain. These enrollments approve no rule. Native counts are **28 accepts / 260 holds**, reviewed 35 / unreviewed 253. The snapshot remains **3,316,962 bytes**; decisions retain exact **453,033 CRLF bytes**, SHA-256 `a68386a3d07f12cdbe0ad1697ac06a09823e981814236f12c9c1667812697174`. Root independently rehashed 19 Ironwood and 29 Wroth/Horizon evidence files, and the combined structural check permits only these three decision changes. The perpetual goal remains active; this is not all board games worldwide. See [the current checkpoint](research/coverage/sep26-prepared-search-identity-progress.md).

Rules-page metadata and names/aliases are now prepared once; the home lookup prepares its lazy index once per successful request. Visible links retain stable ranking, clearing restores original order, and early input remains reconciled. Only large collapses temporarily detach the same list, restoring it in `finally` with the input still connected. Final compiled synthetic measurements use **70 before / 70 final samples**: per-input parsing falls from **881 calls to zero**. At simulated CPU4, combined input/forced-style-layout work falls from **44.5 to 5.9ms** for `a`, **15.2 to 7.4ms** for `TTR`, **11.6 to 1.7ms** for `Azl` and **40.3 to 36.8ms** for clear. Earlier regressions are preserved separately. A **350-sample** comparison and ten CDP samples support conditional staging for style-recalculation cost; the half-catalog threshold is a conservative heuristic. These are not field, physical-device, paint/interaction, ranking, revenue or billing results.

Final app **`npm run verify` passed**: **107 checked files**, zero diagnostics, lint, **112 unit tests**, content validation, **946 pages** and **18,418 internal links**. Canonicals, metadata, structured data, sitemap parity, private-content exclusion, headers and budgets passed. Three unrelated untracked growth source/test files contribute three checked files and seven tests; all five unrelated growth/marketing files remain untouched and outside this commit. **`npm run test:release` passed** the complete configuration/fixture matrix. Evidence: `artifacts/sep26-search-staged-final-{verify,release}.log`.

The focused browser run passed **73 of 75 cases** in **3.3 minutes** across Chromium, Firefox and WebKit. Both failures were new tests assuming Tab reaches anchors in this Windows WebKit runner; ranking/count/clear-order assertions had passed. Retained traces and direct checks show both Tab and Alt+Tab skip anchors to native summaries or body. Chromium/Firefox retain sequential Tab checks; WebKit now checks native anchor focus, keyboard indicator/ring geometry and Enter activation. Native sequential link traversal in this runner is not claimed. **All six corrected keyboard cases passed across the three engines**, in **1.1 minutes**, with unchanged application code and time limits. Post-correction type checking passed all **107 files** with zero diagnostics; selected test lint passed. Initial traces/logs and retry results use separate `sep26-search-staged-final-browser-results/` and `sep26-search-staged-keyboard-retry-results/` folders. No uninterrupted 75/75 or full-suite browser pass is asserted.

Root opened actual compiled mobile F/Z captures in normal/reduced motion and an actual WebKit F capture. Focus rings are complete, targets remain **44px**, and measured document overflow is absent. The first -6px proposal still clipped F by one pixel; the final mobile-only -8px inset resolves both sampled edges. Actual directory searches show the three new names with no checked rule yet. A bounded 18-case page audit plus six keyboard/no-JavaScript contexts covered article, method and directory journeys. An initial ignored diagnostic helper incorrectly expected one Azul match; the corrected three-match expectation passed without an application change. Three exact manual proposals and five further native identity leads are prepared for independent review and remain unapproved.

Final output: **975 files**, **42 shelves**; landing **7,931 raw / 2,232 gzip bytes**, full lazy search **352,260 / 87,778**, rule index **100,471 / 26,194**, largest shelf **114,345 / 6,659**. Conservative gzip budgets: initial JavaScript **110,629 bytes**, Balloon **1,161**, basic home **127,150**. Final build **11.58 seconds** on this host. New distinct size/comparison evidence preserves older measurements. All scoped browser/server sessions are closed; no deployment occurred. [The progress record](research/coverage/sep26-prepared-search-identity-progress.md) contains exact source provenance, experiment history, artifact locations and limits.

## Earlier checkpoint: native editions, complete citations and large-group results — 2026-09-26 UTC

The directory lists **4,981 identities**, with **881 approved edition records** covering **875 sourced identities**; **4,106** await rule research. Seven discovery identities were independently corroborated, and five edition answers were independently reviewed: The 7th Citadel, Stonesaga, Civolution, Magalon and Hick Hack in Gackelwack. Exact edition/mode limits, archived German source custody and approval fingerprints remain explicit. The portable pool stays **361 criteria**, and the public house pool **60 prompts**. The native snapshot now has **25 accepts / 263 holds**; its original **3,316,962 bytes** remain identical to HEAD. Three subsequent semantic accepts preserve Wikidata-only numeric provenance and complete prior holds. These counts do not establish every board game worldwide. See [the exact source and UX checkpoint](research/coverage/sep26-next-native-source-pages-progress.md) and [identity review](research/coverage/sep26-next-native-identity-progress.md).

Public rules-page search now offers a keyboard-usable full-directory recovery link carrying the current bounded Unicode fragment, plus ordinary browsing without JavaScript. The shared directory consumes text entered before its module initializes. Each recognized reviewed PDF source exposes every distinct cited page in reviewed order through ordinary links of at least **44px**, retaining complete-source access and original query bytes. Existing publisher fragments and HTML/viewer URLs are preserved. Page fragments are viewer hints, not a guaranteed jump. Large-group results now occupy their own space beside the reordered draw controls; visual results reserve completion space and remain visible during scrolling, with a single live announcement.

The source/navigation checkpoint passed **`npm run verify`** with **103 checked files** and **102 unit tests**. After the name-sizing correction, the final shared-tree run passed with **106 checked files**, zero diagnostics, lint and **109 unit tests**: the same 102 checkpoint tests plus seven unrelated untracked growth-report tests. Those three growth source/test files and their marketing notes are outside this commit and remain untouched. Content validation, **946 preview pages**, **18,415 internal links**, canonicals, unique metadata, structured data, sitemap parity, private-content exclusion, security/cache headers and static budgets passed. The final **`npm run test:release`** also passed preview/production/fixture/disabled-mode checks and missing-live-config rejection. The first combined verify found two optional-array-index typing errors in the new provenance assertion; corrected guards retain the exact history/record checks. Evidence: ignored `artifacts/sep26-next-combined-{final-verify,corrected-verify,final-release}.log` and `artifacts/sep26-next-names-final-{verify,release}.log`.

The initial complete browser run passed **312 of 315 cases** in **28.1 minutes**, before the later semantic/historical enrollment and result/startup fixes. Its failures were a real development-directory early-input race, a WebKit 61-draw question-pool timeout, and a WebKit More methods click stall before the glow draw. The startup fix is covered by delayed real-module tests requiring reconciliation without another input event. The question-pool test retains all 61 real clicks, 60 uniqueness checks and repeat-boundary check at the same 90-second limit; it removes 59 redundant label assertions and separates unrelated development listing. The glow helper/test and its time limit remain unchanged. Initial logs, traces and screenshots remain preserved under ignored `artifacts/sep26-next-native-source-pages-browser.log` and its three `initial-*-failure/` folders. An uninterrupted full-suite pass on the final application is not claimed.

**All 84 content/search cases passed across Chromium, Firefox and WebKit** in **3.5 minutes**, before the final name-sizing correction, covering both startup cases, the complete question pool, the new articles, Unicode keyboard handoff, stale failures, unavailable indexes, no JavaScript, source-page order and exact Google URL preservation. A subsequent selected motion run passed **33 of 36 cases** in **8.1 minutes**. Two Firefox desktop cases stopped before picking because inner textarea `scrollHeight` was assigned to a bordered outer height. Independent fresh measurements found all 50 fields at outer78/client76/scroll79; complete characters were visible, but their bottom inset was reduced. Autosizing now includes both borders and rounds upward, retaining the unchanged +2 predicate. The third failure captured Firefox cancelling the remembered TableReveals lazy import when Game rules navigation began 25.108ms after the import request. The development journey now verifies the remembered Shell Game radio and three-player preview before its next navigation, retaining console assertions and time limits. Exact exception properties were not retained; an animation-code exception is not inferred. All three failures remain preserved under ignored `artifacts/sep26-next-combined-motion-firefox-*failure/`. **All 48 final motion/name/accessibility cases then passed across Chromium, Firefox and WebKit in 8.7 minutes**, including all three previously failing Firefox paths, the unchanged glow case, completion reservation, reduced motion, Unicode/name reflow and core accessibility. Evidence: ignored `artifacts/sep26-next-combined-focused-content-browser-retry.log`, `artifacts/sep26-next-combined-focused-motion-browser-debug.log` and `artifacts/sep26-next-names-final-motion-browser.log`.

Two automatic server launches timed out before any cases ran. A manual identical command was ready in 567ms; the unchanged content retry passed. The diagnostic motion retry recorded Astro ready in 738ms and a successful first home GET after roughly 45.5 seconds; Astro's printed request time was 1,593ms. No unsupported cause or timeout/config change is claimed. The installed checker already excludes retained artifacts and release fixtures; both inspected earlier checks reported the same 103 files. Read-only reports remain ignored under `artifacts/sep26-webserver-readiness/` and `artifacts/sep26-next-check-scope-review.md`.

Root opened Magalon/Hick Hack mobile and Hick Hack desktop article captures. Their source actions measure at least **44px** with no horizontal overflow at 320px/1366px; rules-to-directory handoff is also 44px and retains the query. Earlier complete Citadel/Stonesaga/Civolution/Brass and mobile handoff captures were inspected. Static Instant/Quick captures cover **eight 50-name cases** before the final name-sizing correction, at both widths and both motion preferences: one announcement, all 50 names, no horizontal overflow, both winner/button fully visible and **zero overlap**. Mobile winner top moved from 2,183px to 255px; desktop overlap was removed. Root opened mobile Instant and desktop Quick. Fresh rebuilt Firefox captures then verified all 50 exact names at 320px/1366px under normal/reduced motion, immediately, after fonts and after settling, without style overrides. All sizing predicates passed; desktop outer80/client78/scroll79 retains a disclosed one-pixel reporting difference, while mobile outer58/client56/scroll56 has none. Root opened the desktop first-name and mobile last-name PNGs; complete final characters remain visible. Isolated servers closed and before evidence remains unchanged. Evidence: ignored `artifacts/sep26-next-combined-final-captures/`, `artifacts/sep26-large-result-{measure,after}/` and `artifacts/sep26-firefox-name-sizing{,-after}/`. No new physical-device, manual screen-reader or Lighthouse result is claimed.

Final local output: **975 files**, **42 shelves**, directory landing **7,931 raw / 2,231 gzip bytes**, lazy search **352,109 raw / 87,741 gzip bytes**, largest shelf **114,345 raw / 6,659 gzip bytes**. Conservative gzip budgets: initial JavaScript **110,346 bytes**, Balloon **1,161 bytes**, basic home **126,852 bytes**. Final build time was **23.15 seconds** on this host. Final sizes are separately recorded in ignored `artifacts/sep26-next-names-final-size-audit.json`; the earlier size audit filename was reused before this last correction and does not preserve its first output. These are local observations, not field performance, ranking, revenue or billing results. No deployment occurred. The perpetual goal remains active. Nippon's final-rule equivalence, Rüsselbande's proof/final equivalence, Wroth/Beyond the Horizon identity bridges and Uprising's other modes remain open. A subsequent official Super Meeple source supplies Ironwood's literal numeric bridge; its complete evidence passed independent root rehash/entity/domain review, but it remains held pending a separate enrollment checkpoint.

Final Git fingerprint checking found that the first local commit normalized the decision file from reviewed CRLF to LF while retaining identical JSON. Git attributes now preserve exact bytes for both native inputs: the snapshot blob retains its original 3,316,962 bytes/hash, and the 417,542-byte decision blob matches the reviewed `88fdda05…b5b2621` hash. Explicit restaging and a conditional fingerprint gate passed, followed by offline validation of 25 accepts/263 holds. Failed and corrected byte-audit logs remain ignored under `artifacts/sep26-final-git-fingerprint*.log`. This representation correction changes no application fields.

## Six modern editions and search recovery checkpoint — 2026-09-26 UTC

The directory lists **4,974 identities**, with **876 approved edition records** covering **870 sourced identities**; **4,104** still await rule research. CuBirds, SKYJO, Samurai Sword, Nucleum, Spicy and Dune: Imperium – Uprising were independently reviewed against exact publisher-linked or publisher-hosted source bytes, complete relevant pages and public record fields. Every record has an explicit identity override and exact approved revision. Only Spicy enters the portable pool, now **361 criteria**. Uprising currently covers three/four players; other modes remain held. Intake records retain draft history. The 288-candidate native snapshot and its 18 accepted identities remain unchanged. These counts do not establish every board game worldwide. See [the exact source and UX checkpoint](research/coverage/sep26-modern-search-progress.md).

Home lookup now offers an ordinary full-directory link and carries a bounded Unicode search in a URL fragment. The directory restores that query without putting it in HTTP request queries or changing its canonical URL. Stale results/errors cannot overwrite newer queries, and failed lookup retains usable static browsing. Both exact public JSON indexes emit **`public, max-age=300, must-revalidate`**, with matching host policies audited from the emitted `_headers`; hashed assets retain immutable caching. This permits five-minute reuse and introduces the same publication-freshness delay. No measured bill reduction is asserted.

The final application passed **`npm run verify`**: **99 Astro files**, zero diagnostics, lint, **59 unit tests**, content validation, **941 preview pages** and **18,346 internal links**. Canonicals, unique metadata, structured data, sitemap parity, private-content exclusion, security/cache headers and static budgets passed. **`npm run test:release`** passed the preview/production/fixture/disabled-mode matrix and missing-live-config rejection. Evidence: ignored `artifacts/sep26-modern-search-polished-{verify,release}.log`.

The complete Chromium/Firefox/WebKit browser run passed **279 of 282 cases** in 17.9 minutes. Its three failures checked the former home no-match wording; traces showed the intended new directory-recovery message. The assertions now check that message and the query-bearing link. After a final status-spacing polish, **all 24 focused home-layout/search cases passed across all three engines** (45.5 seconds), including those corrected cases, keyboard handoff, Unicode, delayed responses, late failure/clear, unavailable indexes, malformed fragments and no JavaScript. An uninterrupted 282/282 result for the final stylesheet is **not** claimed. Evidence: ignored `artifacts/sep26-modern-search-browser.log`, `artifacts/sep26-modern-search-copy-assertion-results/` and `artifacts/sep26-modern-search-polished-focused-browser.log`.

An earlier broad run was stopped after detecting the stale wording assertion and a new test's hidden-status locator. Clearing the query correctly hid the empty live status; the test now explicitly includes and checks the hidden element. All 18 search-journey cases passed after that correction. Earlier verification also found stale pending-rule assumptions and a TypeScript tuple typing issue in tests; both were corrected before the final gate. Initial logs, traces and screenshots remain preserved under ignored `artifacts/sep26-modern-search-initial-*`, `artifacts/sep26-modern-search-initial-browser-results/` and `artifacts/sep26-modern-search-typecheck-correction.log`.

Fresh static captures were opened and inspected: SKYJO, Samurai Sword and Nucleum at 320px, Uprising at desktop width, and search/directory handoff at mobile and desktop widths. Eight article geometries showed no horizontal overflow and source targets of at least 44px. The final home no-match captures at **320px and 1366px** show a **12px input/status gap**, a **44px directory link** and no horizontal overflow; the restored directory query also fits. Actual static GETs returned the exact five-minute cache headers for both JSON paths and no cache policy for the home HTML. Evidence: ignored `artifacts/sep26-modern-search-layout.json`, `artifacts/sep26-search-polished-layout.json` and their article/search PNGs. No new physical-device, manual screen-reader or Lighthouse result is claimed.

Final local output: **971 files**, **42 shelves**, directory landing **7,931 raw / 2,229 gzip bytes**, lazy search **351,197 raw / 87,441 gzip bytes**, largest shelf **114,359 raw / 6,678 gzip bytes**. Conservative gzip budgets: initial JavaScript **110,355 bytes**, Balloon **1,161 bytes**, basic home **126,738 bytes**. The final build took **7.93 seconds** on this host. These are local artifact measurements, not field performance, ranking, revenue or billing results. No deployment occurred. The perpetual goal remains active; source/identity proposals, further search recovery and complete cited-page navigation are queued for independent review.

## Six editions, native identities and mobile layout checkpoint — 2026-09-26 UTC

The directory now lists **4,974 identities**, with **870 approved edition records** covering **864 sourced identities**; **4,110** still await rule research. Six independently reviewed editions were added: FFG Samurai (2015), Queen Shogun (2006), John Company: Second Edition (second printing 2023), Summoner Wars: Second Edition (v1.2), London: Second Edition (2017), and the undated Pandasaurus Kwanchai Moriya edition of The Game. Each has an explicit identity mapping and exact approval fingerprint. Only Samurai joins the separately approved portable mix, now **360 criteria**. Setup, action order, fixed roles and inapplicable opening ties remain distinct. See [the source and UX review](research/coverage/sep26-six-native-mobile-progress.md).

A complete supplemental Wikidata query preserves **288** numeric IDs without English-labeled attached items, their full entity evidence and native or language-neutral titles. Eighteen primary-corroborated identities are enrolled; **270 remain held**, including unreviewed candidates. Exact snapshot bytes, canonical statement/entity hashes, selected title provenance, entity revisions and actual nondeprecated P2339 statement IDs are checked before enrollment. No rule prose or edition answer transfers with an identity. The original English snapshot is unchanged. A Git attribute preserves the new snapshot's original CRLF bytes on every platform; the staged blob exactly matches the **3,316,962-byte** working file and SHA-256 `d13f929adfeb2a0b8a163497b5b45ff3627c7914e7381c86a74152fa575e31b0`. The importer offline validator and tamper checks passed. See [the native-title review](research/coverage/native-title-progress.md). These counts do not establish every board game worldwide.

Twelve-player cards, dice and coins now use three mobile columns, smaller pieces and shorter travel. At **320×844**, measured stage heights fell from **881→531px**, **977→547px** and **965→599px**, respectively. The last winning seat is fully visible at both 320px and 390px widths without horizontal overflow. A hidden placeholder outside the live region reserves the final announcement's height; the single accessible winner remains sticky for taller scenes. Normal and reduced motion retain one reveal scroll. Larger groups may still need scrolling. Rule articles place official opening ties directly below the answer and provide cited-page/full-source actions with at least **44px** target height; HTML sources use an accurate caption.

The frozen final application snapshot passed **`npm run verify`**: **96 Astro files**, zero diagnostics, lint, **50 unit tests**, content validation, **935 preview pages** and **18,269 internal links**. Canonicals, unique metadata, structured data, sitemap parity, private-content exclusion, security headers and static budgets passed. **`npm run test:release`** passed the complete preview/production/fixture/disabled-mode matrix and missing-live-config rejection. **All 261 browser cases passed in one uninterrupted Chromium/Firefox/WebKit run** (16.3 minutes), including first click/tap, secure-RNG recovery, unavailable-module fallbacks, keyboard/reduced motion, catalog scope and mobile layouts. Evidence: ignored `artifacts/sep26-final-six-native-verify.log`, `artifacts/sep26-corrected-release.log` and `artifacts/sep26-final-six-native-browser.log`.

Earlier attempts found an initial placeholder inside the live announcement, which could satisfy result assertions before settlement. It was moved outside that region, then the unchanged dice-fade check passed in all engines. A focused cold-load test also matched React's hidden streamed server copy alongside the active coin scene. Trace inspection showed one visible scene and a separate hidden island container; the test now scopes the mounted picker and requires exactly one visible scene. All **12 focused cases passed** after these corrections. Failed attempts remain preserved in ignored logs and `artifacts/sep26-{placeholder-initial,cold-preview-failures}/`; no failure is omitted from this history.

Fresh static captures of Shogun/Backgammon at 320px, John Company at desktop width, the CuBirds pending disclosure at 390px, and the corrected three twelve-player results at 320px were opened and inspected. Source actions and opening ties were legible, with no observed overlap or horizontal overflow. Geometry measurements additionally checked four article templates at both 320px and 1366px and all three dense scenes at 320px/390px. Evidence: ignored `artifacts/sep26-article-layout.json`, `artifacts/dense-motion-after.json`, and the accompanying article/dense PNGs. No new Lighthouse, physical-device or manual screen-reader result is claimed.

Local output measurements: **964 files**, **42 shelves**, directory landing **7,931 raw / 2,231 gzip bytes**, lazy search index **350,561 raw / 87,240 gzip bytes**, largest shelf **114,359 raw / 6,679 gzip bytes**. Conservative gzip budgets: initial JavaScript **109,749 bytes**, Balloon **1,161 bytes**, basic home **126,097 bytes**. This build took **13.27 seconds** on the shared host. These are local artifacts, not field performance, ranking, revenue or billing results. No deployment occurred. The perpetual goal remains active; the next independent source batch and home-search-to-directory bridge are queued for review.
