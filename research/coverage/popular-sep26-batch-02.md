...
The final rebased batch passed content validation (966 approved rules, 373 usable random rules), Astro checks with zero errors/warnings/hints, ESLint, and all 153 tests in 16 files. The production build passed its audits across 1,127 pages and 30,509 internal links, using the live canonical origin, owner contact and Cloudflare privacy disclosure. The main-push hold was lifted after the shared release passed production checks. No application UI files were changed by this batch.

Pushed as 7bc04ca and verified live on 2026-09-26: all nine new edition pages returned HTTP 200 with indexable canonical URLs; the 966-record home search index includes them and the repaired Clever aliases; the 5,008-entry directory index links each exact edition correctly. Home, directory, robots and sitemap also returned HTTP 200. The shared release routes single editions straight to their rule pages and retains chooser pages for multiple editions; this batch preserves that routing.
