...
Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 864 researched rule records in total; 858 of 4,956 inventory identities match a researched edition and 4,098 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 864 rule identifiers have approved catalog copies, each with its own reviewed revision; intake copies can retain their original draft history. The random mix has 359 separately reviewed portable criteria. Research coverage, approval and deployment remain separate.
