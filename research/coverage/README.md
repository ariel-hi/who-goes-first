...
The supplemental `wikidata-native-title-leads.json` preserves all 288 current numeric P2339 IDs without an English-labeled attached item, including full entity revisions, native/P1476 titles, aliases and claims. `wikidata-native-title-decisions.json` accepts 31 identities corroborated by publisher or author sources and holds 257, including already-listed and unresolved candidates: 38 reviewed and 250 unreviewed. Accepted identities alone join the active inventory, bringing it to **4,987 identities**. Snapshot bytes, reconstructed statement/entity hashes, exact titles and nondeprecated ID statements are validated before enrollment. Ironwood and Beyond the Horizon have exact numeric links on their saved publishers' domains; Wroth has an explicit Frosted original/localized partnership bridge. Northgard: Uncharted Lands, ImmunoWars and Townsfolk Tussle add primary semantic corroboration while retaining Wikidata-only numeric provenance. ImmunoWars and Townsfolk Tussle now have exact source-reviewed editions; Northgard still awaits a primary manual. See [the manual checkpoint](sep26-enrolled-manuals-readable-roster-progress.md). Complete prior holds and edition limits remain. Run `python scripts/import-wikidata-native-title-leads.py --validate`; see [the current three-primary review](sep26-three-primary-identities-progress.md), [native-title-progress.md](native-title-progress.md), [Ironwood](sep26-ironwood-identity-progress.md) and [Wroth/Horizon](sep26-two-identity-progress.md). This live pool does not prove the membership of the earlier snapshot's unlabeled count.

Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 886 researched rule records in total; 880 of 4,987 inventory identities match a researched edition and 4,107 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 886 rule identifiers have approved catalog copies, each with its own reviewed revision; intake copies can retain their original draft history. The random mix has 361 separately reviewed portable criteria. Research coverage, approval and deployment remain separate.
