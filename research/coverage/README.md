...
Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 881 researched rule records in total; 875 of 4,984 inventory identities match a researched edition and 4,109 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 881 rule identifiers have approved catalog copies, each with its own reviewed revision; intake copies can retain their original draft history. The random mix has 361 separately reviewed portable criteria. Research coverage, approval and deployment remain separate.
