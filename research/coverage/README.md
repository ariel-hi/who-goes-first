...
`wikidata-board-games.json` is a 2026-09-26 snapshot of English labels and BoardGameGeek identifiers from [Wikidata P2339](https://www.wikidata.org/wiki/Property:P2339), whose [structured data is CC0](https://www.wikidata.org/wiki/Wikidata:Data_access). The snapshot records the exact query, source URL, retrieval date, QIDs, statement hash and review flags. Run `python scripts/import-wikidata-board-games.py --validate` to check it, or run without `--validate` to refresh from the official SPARQL endpoint. The active inventory adds 3,572 unflagged, name-distinct leads from this snapshot to the original 1,368, for 4,940 listed identities. Another 205 new IDs have review flags, and some otherwise unflagged leads collide with original names. These remain research leads, not published identities or rules. Wikidata explicitly marks P2339 incomplete, so this inventory is not all games worldwide.
...
Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 861 researched rule records in total; 855 of 4,940 inventory identities match a researched edition and 4,085 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 861 researched exact revisions are approved for the catalog; 359 portable criteria are eligible for the random mix. Research coverage, approval and deployment remain separate.
