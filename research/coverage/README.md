...
Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 887 researched rule records in total; 881 of 4,988 inventory identities match a researched edition and 4,107 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 887 rule identifiers have approved catalog copies, each with its own reviewed revision; intake copies can retain their original draft history. The random mix has 361 separately reviewed portable criteria. Research coverage, approval and deployment remain separate.
