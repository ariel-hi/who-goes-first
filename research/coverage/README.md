...
Run `npm run content:coverage` or open `/dev/coverage/` to compare the complete inventory with researched editions. At this checkpoint there are 603 researched rule records in total; 581 inventory identities match a researched edition and 739 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 603 researched exact revisions are approved for the catalog; 238 portable criteria are eligible for the random mix. Research coverage, approval and deployment remain separate.
