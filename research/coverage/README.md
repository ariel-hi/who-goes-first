...
The supplemental `wikidata-native-title-leads.json` preserves all 288 current numeric P2339 IDs without an English-labeled attached item, including full entity revisions, native/P1476 titles, aliases and claims. `wikidata-native-title-decisions.json` accepts 18 identities corroborated by publisher or author sources and holds 270, including already-listed and unresolved candidates. Accepted identities alone join the active inventory, bringing it to **4,974 identities**. Snapshot bytes, reconstructed statement/entity hashes, exact titles and nondeprecated ID statements are validated before enrollment. Run `python scripts/import-wikidata-native-title-leads.py --validate`; see [native-title-progress.md](native-title-progress.md). This live pool does not prove the membership of the earlier snapshot's unlabeled count.

Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 876 researched rule records in total; 870 of 4,974 inventory identities match a researched edition and 4,104 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 876 rule identifiers have approved catalog copies, each with its own reviewed revision; intake copies can retain their original draft history. The random mix has 361 separately reviewed portable criteria. Research coverage, approval and deployment remain separate.

CuBirds, SKYJO, Samurai Sword, Nucleum, Spicy and Dune: Imperium – Uprising add six independently reviewed editions. Their explicit identity mappings preserve component-dependent openings, later-round changes and edition/player-count limits. Only Spicy joins the portable mix. Home search now carries bounded queries into the full directory, with static browsing and failure recovery; the two public lookup indexes have audited five-minute cache policies. See [the modern-source and search checkpoint](sep26-modern-search-progress.md). Four further identity proposals and the next source batches remain unapproved and unenrolled.
