...
The supplemental `wikidata-native-title-leads.json` preserves all 288 current numeric P2339 IDs without an English-labeled attached item, including full entity revisions, native/P1476 titles, aliases and claims. `wikidata-native-title-decisions.json` accepts 28 identities corroborated by publisher or author sources and holds 260, including already-listed and unresolved candidates. Accepted identities alone join the active inventory, bringing it to **4,984 identities**. Snapshot bytes, reconstructed statement/entity hashes, exact titles and nondeprecated ID statements are validated before enrollment. Ironwood and Beyond the Horizon now have exact numeric links on their saved publishers' domains; Wroth has an explicit Frosted original/localized partnership bridge and retains Wikidata-only numeric provenance. Complete prior holds and edition limits remain. Run `python scripts/import-wikidata-native-title-leads.py --validate`; see [native-title-progress.md](native-title-progress.md), [the preceding identity review](sep26-next-native-identity-progress.md), [Ironwood](sep26-ironwood-identity-progress.md) and [Wroth/Horizon](sep26-two-identity-progress.md). This live pool does not prove the membership of the earlier snapshot's unlabeled count.

Run `npm run content:coverage` or open `/dev/coverage/` to compare the active inventory with researched editions. At this checkpoint there are 884 researched rule records in total; 878 of 4,984 inventory identities match a researched edition and 4,106 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 884 rule identifiers have approved catalog copies, each with its own reviewed revision; intake copies can retain their original draft history. The random mix has 361 separately reviewed portable criteria. Research coverage, approval and deployment remain separate.

The [three-manual checkpoint](sep26-three-manual-progress.md) adds exact reviewed Ironwood English two-player, Wroth English v1.0 standard 2–4-player and Beyond the Horizon French 2–4-player answers. Twenty complete pages were independently inspected by root. The French source is explicitly summarized in English, and the Wroth viewer retains readable page references without unsupported PDF shortcuts. Native identity decisions and their original byte hashes are unchanged.

Rules and home lookup now prepare search names/aliases once. Visible links keep stable ranking, clearing restores catalog order, and only large visibility collapses stage the same list outside the DOM for updates. The mobile alphabet retains its focus ring at the scroll edges. Final app verification passed 107 checked files, 112 unit tests, 946 pages and 18,418 internal links; release checks passed. The focused browser run passed 73/75, followed by 6/6 corrected keyboard cases across all three engines. The WebKit runner's native Tab-to-link limitation and the original traces remain explicit. Three exact manual proposals and five further native identity leads await independent review. See [the current search and identity checkpoint](sep26-prepared-search-identity-progress.md).
