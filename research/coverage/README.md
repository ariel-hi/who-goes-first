...
Run `npm run content:coverage` or open `/dev/coverage/` to compare the complete inventory with researched editions. At this checkpoint there are 560 researched rule records in total; 538 inventory identities match a researched edition and 782 still need primary-source research. Matching a title does not prove every edition agrees. The owner has delegated editorial review to Codex. All 560 researched exact revisions are approved for the catalog; 212 portable criteria are eligible for the random mix. Research coverage, approval and deployment remain separate.
