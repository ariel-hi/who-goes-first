...
A local Astro + strict TypeScript site, with one React picker island and static editorial pages. The full game-rule compendium is still in progress. No public deployment has been made. The owner has authorized Codex to review sources and approve exact content revisions; 228 game rules have completed that pass, with 168 standalone criteria in the random mix. See `CONTENT_REVIEW.md` for the current review process.
...
- Unbiased secure randomness; immutable outcomes; Instant, Quick, Spinner, Card Draw, Balloon Rise, Towers, Shortest Match, Dice Roll, and Marble Race; skip and interruption handling; muted-by-default optional synthesized sound. Decorative methods load on use.
- Finished scenes stay visible until another pick, a player edit, or a method change. Spinner uses a compact color-and-name legend in place of the editable roster during a draw; Edit players restores the names. Towers share the same build animation and leave visible fallen blocks; popped balloons leave scraps. Method pages share the home picker's heading and preselect their method.
- Player colors follow their IDs across all methods. Each draw gets fresh balloon timings, overlapping card flips, and independent marble paths. Shortest Match uses wooden matches with colored heads; its existing `/methods/straws/` URL and saved preference remain compatible.
...
Public builds contain no unapproved editorial content. There are 430 researched rule records, of which 228 have completed editorial approval, plus 60 published original house questions (the six earlier prompt drafts are preserved). Publisher batches add 86 Gamewright entries, 13 Blue Orange edition entries, nine Rio Grande entries, six Zoch entries, fourteen Thames & Kosmos entries, twenty AEG entries, thirteen Allplay entries, six AMIGO entries, two Brain Games entries, nine Czech Games Edition entries, one WizKids entry, three Fowers entries hosted by Skellig Games, one Dire Wolf entry, one Dice Hate Me entry, four Alley Cat Games entries, one Red Raven Games second-edition entry, five R&R Games entries, three Board&Dice entries, three Ludonova entries, five Gigamic entries, three Sorry We Are French entries, one Funnyfox entry, eight HABA entries, four Fantasy Flight Games entries, six Queen Games entries, five Schmidt Spiele entries, eight dV Giochi entries, five Hutter Trade publisher/distributor entries, seven ABACUSSPIELE entries, seven Looney Labs entries, seventeen Pegasus-hosted entries, five Renegade/creator entries, one Wizards of the Coast entry, five Z-Man entries, nine new Days of Wonder edition entries, seven Repos entries, eight Pandasaurus entries, one Portal-distributed entry, eight Ravensburger entries, four Deep Print entries, two Cocktail Games entries, three Helvetiq entries, two Horrible Guild entries, nine Eagle-Gryphon editions, one Ludonaute entry, one 2F-Spiele entry, six Mindclash records (including two Perseverance episodes), nine ThunderGryph records, three Sinister Fish records, six Gamelyn official Dized records, ten other official Dized records, two Osprey records, three Genius Games records, four Capstone records, three Atlas records, three North Star records, three Thunderworks records, two Underdog records, three further Next Move records, one additional Road to Infamy record, two Snowdale Design records, three Steve Jackson Games records, one each from Libellud, Rebel Studio and Keymaster, and one each from Keen Bean Studio, Road to Infamy, Weird Giraffe Games and EmperorS4 from primary manuals. All 430 are searchable locally. The random mix contains only the 168 individually reviewed portable criteria; cooperative role assignments, simultaneous play, component-dependent setup and random-only instructions remain outside it. Distinct Spin Circus, Valley of the Kings, Ticket to Ride: Europe and Incan Gold editions retain their different starting instructions.

The separate 1,320-game discovery inventory contains identities, not imported rules: 408 of those identities now have a researched edition here; 912 remain. Seventeen researched games are outside that inventory; Spin Circus, Valley of the Kings, Ticket to Ride: Europe and Incan Gold each have two edition records. Perseverance: Castaway Chronicles – Episodes 1 & 2 has two separately researched episode answers under one inventory identity. Explicit identity overrides keep unrelated games such as the Gamewright and Allplay games named Chomp and Big Top separate in coverage counts. Completing source research remains implementation work; the inventory does not satisfy the request for a full compendium. Edition-specific entries distinguish card-assigned or simultaneous starts from rules that call for a random player. Static builds contain the 228 reviewed catalog entries. Preview builds remain non-indexable; approval is separate from public deployment.
...
Five existing R&R Games drafts also complete review, with four portable criteria. See `research/coverage/rnr-review-progress.md`. The current local directory has 430 researched records; 228 exact revisions are approved and 168 criteria are in the random mix.
