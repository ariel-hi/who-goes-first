...
1. Create/connect the owner's chosen repository and Pages project. Select a production branch. Use Node 22.23.2 (or compatible Node 22 maintenance), build command `npm run build`, output directory `dist`. Pages installs locked dependencies automatically; do not repeat `npm ci` in its build command. A standalone host without automatic installation must install dependencies first.
...
5. Keep the local receipt in ignored `artifacts/indexnow/receipts.jsonl`. HTTP 200 means received; HTTP 202 means received with key validation pending. Neither proves indexing or traffic. Matching page-content hashes already acknowledged in this checkout prevent unchanged repeats. This local ledger is not shared across machines, so consult previous release receipts before sending elsewhere.
